...
class TestEnrichmentCSS(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative CSS file.
    """

    def test_css_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a CSS file.
        """
        print("\n--- Testing Live CSS Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.css"
        self.assertTrue(source_file.exists(), "Source file 'source.css' not found.")
        ...
        print("✅ FULL PASS: CSS enrichment test successful.")
