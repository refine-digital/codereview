...
class TestEnrichmentHTML(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative HTML file.
    """

    def test_html_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for an HTML file.
        """
        print("\n--- Testing Live HTML Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.html"
        self.assertTrue(source_file.exists(), "Source file 'source.html' not found.")
        ...
        self.assertIn("#_LOCATIONMAP", full_analysis_text, "Placeholder '#_LOCATIONMAP' not mentioned in the analysis.")
        self.assertIn("#_BOOKINGFORM", full_analysis_text, "Placeholder '#_BOOKINGFORM' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: HTML enrichment test successful.")
