...
class TestEnrichmentJS(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative JavaScript file.
    """

    def test_js_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a JavaScript file.
        """
        print("\n--- Testing Live JavaScript Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.js"
        self.assertTrue(source_file.exists(), "Source file 'source.js' not found.")
        ...
        self.assertIn("reserve_selected_userfields", full_analysis_text, "Function 'reserve_selected_userfields' not mentioned in the analysis.")
        self.assertIn("serializeObject", full_analysis_text, "Function 'serializeObject' from the bundled plugin not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: JavaScript enrichment test successful.")
