...
class TestEnrichmentJSON(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative JSON file.
    """

    def test_json_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a JSON file.
        """
        print("\n--- Testing Live JSON Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.json"
        self.assertTrue(source_file.exists(), "Source file 'source.json' not found.")
        ...
        self.assertIn("name", full_analysis_text, "Key 'name' not mentioned in the analysis.")
        self.assertIn("autoload", full_analysis_text, "Key 'autoload' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: JSON enrichment test successful.")
