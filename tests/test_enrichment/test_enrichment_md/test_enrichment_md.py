...
class TestEnrichmentMD(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative Markdown file.
    """

    def test_md_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a Markdown file.
        """
        print("\n--- Testing Live Markdown Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.md"
        self.assertTrue(source_file.exists(), "Source file 'source.md' not found.")
        ...
        self.assertIn("install", full_analysis_text, "Term 'install' not mentioned in the analysis.")
        self.assertIn("font", full_analysis_text, "Term 'font' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: Markdown enrichment test successful.")
