...
class TestEnrichmentPHP(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative PHP file.
    """

    def test_php_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a PHP file.
        """
        print("\n--- Testing Live PHP Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.php"
        self.assertTrue(source_file.exists(), "Source file 'source.php' not found.")
        ...
        # These task IDs are defined in the main config.yaml
        ...
        self.assertIn("EM_Pro", full_analysis_text, "Class 'EM_Pro' not mentioned in the analysis.")
        self.assertIn("emp_load_plugin_textdomain", full_analysis_text, "Function 'emp_load_plugin_textdomain' not mentioned in the analysis.")
        self.assertIn("emp_activate", full_analysis_text, "Function 'emp_activate' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: PHP enrichment test successful.")
