# File: tests/test_enrichment/test_po/test_enrichment_po.py
# Version: 1.0.0
import unittest
import sys
import json
...
class TestEnrichmentPO(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative PO file.
    """

    def test_po_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a PO file.
        """
        print("\n--- Testing Live PO Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.po"
        self.assertTrue(source_file.exists(), "Source file 'source.po' not found.")
        ...
        self.assertIn("Coupon Code", full_analysis_text, "msgid 'Coupon Code' not mentioned in the analysis.")
        self.assertIn("booking", full_analysis_text, "Term 'booking' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: PO enrichment test successful.")
