...
class TestEnrichmentSH(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative Shell script file.
    """

    def test_sh_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a Shell script file.
        """
        print("\n--- Testing Live Shell Script Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.sh"
        self.assertTrue(source_file.exists(), "Source file 'source.sh' not found.")
        ...
        self.assertIn("php ./merge.php", full_analysis_text, "Command 'php ./merge.php' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: Shell script enrichment test successful.")
