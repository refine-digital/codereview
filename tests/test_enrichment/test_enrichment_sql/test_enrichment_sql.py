...
class TestEnrichmentSQL(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative SQL file.
    """

    def test_sql_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a SQL file.
        """
        print("\n--- Testing Live SQL Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.sql"
        self.assertTrue(source_file.exists(), "Source file 'source.sql' not found.")
        ...
        self.assertIn("users", full_analysis_text, "Table 'users' not mentioned in the analysis.")
        self.assertIn("id", full_analysis_text, "Column 'id' not mentioned in the analysis.")
        self.assertIn("username", full_analysis_text, "Column 'username' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: SQL enrichment test successful.")
