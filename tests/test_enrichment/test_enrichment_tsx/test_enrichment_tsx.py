# File: tests/test_enrichment/test_enrichment_tsx/test_enrichment_tsx.py
# Version: 1.0.0
import unittest
import sys
import json
...
class TestEnrichmentTSX(unittest.TestCase):
    """
    Tests the live AI enrichment process for a representative TSX file.
    """

    def test_tsx_enrichment_structure_and_relevance(self):
        """
        Verifies that the enrichment process produces a structurally correct
        and contextually relevant analysis for a TSX file.
        """
        print("\n--- Testing Live TSX Enrichment ---")
        
        source_file = Path(__file__).resolve().parent / "source.tsx"
        self.assertTrue(source_file.exists(), "Source file 'source.tsx' not found.")
        ...
        self.assertGreater(len(result["ai_analysis"]["initial_analysis"]),
                           20, "Initial analysis is too short.")
        ...
        self.assertIn("MyComponentProps", full_analysis_text, "Interface 'MyComponentProps' not mentioned in the analysis.")
        self.assertIn("MyComponent", full_analysis_text, "Component 'MyComponent' not mentioned in the analysis.")

        print("✅ PASSED: Keyword spot-checking for relevance successful.")
        print("✅ FULL PASS: TSX enrichment test successful.")
